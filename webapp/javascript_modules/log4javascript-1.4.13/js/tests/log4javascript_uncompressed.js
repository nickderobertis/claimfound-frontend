...
function compareObjectInterface(obj1, obj1_name, obj2, obj2_name, namePrefix) {
  ...
  if (
    obj1 &&
    obj1.prototype &&
    !array_contains(obj1PropertyNames, "prototype")
  ) {
  ...
  for (var j = 0; j < obj1PropertyNames.length; j++) {
    ...
    if (
      (typeof obj1[propertyName] == "function" ||
        typeof obj1[propertyName] == "object") &&
      !(obj1[propertyName] instanceof Array)
    ) {
      var propertyFullyQualifiedName =
        namePrefix == "" ? propertyName : namePrefix + "." + propertyName;
      try {
        if (typeof obj2[propertyName] == "undefined") {
          ...
          throw new Error(
            obj2_name +
              ...
              typeof obj2[propertyName] +
              " when it is type " +
              typeof obj1[propertyName] +
              " in " +
              obj1_name
          );
        ...
        throw new Error(
          "Exception while checking property name " +
            propertyFullyQualifiedName +
            " in " +
            obj2_name +
            ": " +
...
xn.test.suite("log4javascript tests", function(s) {
  ...
  s.test("XmlLayout test", function(t) {
    ...
    t.logger.debug("TEST");
    t.assertRegexMatches(
      /^<log4javascript:event logger="test" timestamp="\d+" level="DEBUG">\s*<log4javascript:message><!\[CDATA\[TEST\]\]><\/log4javascript:message>\s*<\/log4javascript:event>\s*$/,
  ...
  s.test("XmlLayout with exception test", function(t) {
    ...
    t.assertRegexMatches(
      /^<log4javascript:event logger="test" timestamp="\d+" level="DEBUG">\s*<log4javascript:message><!\[CDATA\[TEST\]\]><\/log4javascript:message>\s*<log4javascript:exception>\s*<!\[CDATA\[.*\]\]><\/log4javascript:exception>\s*<\/log4javascript:event>\s*$/,
      t.appender.logMessages[0]
    );
  });
  ...
  var setUpXmlLayoutMillisecondsTest = function(t) {
    ...
    t.loggingEvent = new log4javascript.LoggingEvent(
      t.logger,
      t.date,
  ...
  s.test("XmlLayout seconds/milliseconds test 1", function(t) {
    ...
    var regex = new RegExp(
      '^<log4javascript:event logger="test" timestamp="' +
        ...
        '" level="DEBUG">\\s*<log4javascript:message><!\\[CDATA\\[TEST\\]\\]></log4javascript:message>\\s*</log4javascript:event>\\s*$'
    );
  ...
  s.test("JsonLayout test", function(t) {
    ...
    t.logger.debug("TEST");
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":"TEST"}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout with number type message test", function(t) {
    ...
    t.logger.debug(15);
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":15}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout with object type message test", function(t) {
    ...
    t.logger.debug({});
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":"\[object Object\]"}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout with boolean type message test", function(t) {
    ...
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":false}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout with quote test", function(t) {
    ...
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":"TE\\"S\\"T"}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout with exception test", function(t) {
    ...
    t.logger.debug("TEST", new Error("Test error"));
    t.assertRegexMatches(
      /^{"logger":"test","timestamp":\d+,"level":"DEBUG","url":".*","message":"TEST","exception":.*}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("JsonLayout seconds/milliseconds test 1", function(t) {
    ...
    // Test default (i.e. timestamps in milliseconds) first
    var regex = new RegExp(
      '^{"logger":"test","timestamp":' +
        t.timeInMilliseconds +
        ',"level":"DEBUG","url":".*","message":"TEST"}$'
    );
    t.assertRegexMatches(regex, t.layout.format(t.loggingEvent));
  });
  ...
  s.test("JsonLayout seconds/milliseconds test 2", function(t) {
    ...
    var regex = new RegExp(
      '^{"logger":"test","timestamp":' +
        t.timeInSeconds +
        ',"level":"DEBUG","url":".*","message":"TEST","milliseconds":' +
        t.milliseconds +
        "}$"
    );
    t.assertRegexMatches(regex, formatted);
  ...
  s.test("HttpPostDataLayout URL encoding test", function(t) {
    ...
    t.assertRegexMatches(
      /^logger=test&timestamp=\d+&level=DEBUG&url=.*&message=TEST%20%2B%221%22$/,
      t.appender.logMessages[0]
    );
  });
  ...
  (function() {
    var formatObjectExpansion = log4javascript.evalInScope(
    ...
    s.test("formatObjectExpansion simple object test", function(t) {
      ...
      t.assertEquals(
        formatObjectExpansion(obj, 1),
        "{" + newLine + "  STRING: A string" + newLine + "}"
      );
    });
    ...
    s.test("formatObjectExpansion simple circular object test", function(t) {
      ...
      t.assertEquals(
        formatObjectExpansion(obj, 2),
        "{" +
          newLine +
          "  a: [object Object] [already expanded]" +
          newLine +
          "}"
      );
    });
  ...
  s.test("PatternLayout dates test", function(t) {
    ...
    t.logger.debug("TEST");
    t.assertRegexMatches(
      /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3} \d{2} [A-Z][a-z]{2} \d{4} \d{2}:\d{2}:\d{2},\d{3} \d{2}:\d{2}$/,
      t.appender.logMessages[0]
    );
  });
  ...
  s.test("PatternLayout modifiers test", function(t) {
    var layout = new log4javascript.PatternLayout(
      "%m|%3m|%-3m|%6m|%-6m|%.2m|%1.2m|%6.8m|%-1.2m|%-6.8m|"
    );
    t.appender.setLayout(layout);
    t.logger.debug("TEST");
    t.assertEquals(
      t.appender.logMessages[0],
      "TEST|TEST|TEST|  TEST|TEST  |ST|ST|  TEST|ST|TEST  |"
    );
  });
...
*/
  var testConsoleAppender = function(t, appender) {
    var timeoutCallback = function() {
      ...
      return (
        (windowLoaded
          ? "Timed out while waiting for messages to appear"
          : "Timed out while waiting for window to load") +
        ". Debug messages: " +
        ...
        var checkDom = function() {
          ...
          if (logContainer.hasChildNodes()) {
            if (logContainer.innerHTML.indexOf("TEST MESSAGE") == -1) {
              appender.close();
              t.fail(
                "Log message not correctly logged (log container innerHTML: " +
