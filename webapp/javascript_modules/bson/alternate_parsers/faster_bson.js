...
function calculateElement(name, value) {
  ...
  switch (
    value.constructor /// removed all checks 'isBuffer' if Node.js Buffer class is present!?
  ) {
...
function deserializeFast(buffer, i, isArray) {
  ...
  if (buffer.length < 5)
    return new Error("Corrupt bson message < 5 bytes long"); /// from 'throw'
  var elementType,
    tempindex = 0,
    name;
    ...
    switch (elementType) {
      ...
      case 4: /// = BSON.BSON_DATA_ARRAY
        size =
          ...
          (buffer[i + 1] << 8) |
          (buffer[i + 2] << 16) |
          (buffer[i + 3] << 24); /// NO 'i' increment since the size bytes are reread during the recursion!
      ...
      case 9: /// = BSON.BSON_DATA_DATE:      /// SEE notes below on the Date type vs. other options...
        ...
        object[name] = new Date(
          high * 4294967296 + (low < 0 ? low + 4294967296 : low)
        );
        break;
      ...
      case 18: /// = BSON.BSON_DATA_LONG:  /// usage should be somewhat rare beyond parseResponse() -> cursorId, where it is handled inline, NOT as part of deserializeFast(returnedObjects); get lowBits, highBits:
        ...
        if (size < JS_INT_MAX && size > JS_INT_MIN) object[name] = size;
...
Long.prototype.toNumber = function() {
  return (
    this.high_ * 4294967296 +
    (this.low_ < 0 ? this.low_ + 4294967296 : this.low_)
  );
};
